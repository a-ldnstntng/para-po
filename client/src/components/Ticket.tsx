...
export default function Ticket({
  ...
  const [showDirectionsMobile, setShowDirectionsMobile] = useState(false);
  ...
  const handleCopy = () => {
    ...
    const formatted =
      ...
      `EST. TIME: ~${totalDuration} MINS | TOTAL FARE: PHP ${totalFare.toFixed(2)} (${activeSteps.length} sakay)\n\n` +
      activeSteps
        .map(
          (s, i) =>
            `${i + 1}. [${s.mode.toUpperCase()}] ${s.instruction} (~${s.estimated_duration_min || 15}m | Babaan: ${s.landmark} | ₱${(s.fare_estimate_php || 0).toFixed(2)})`
        )
        .join('\n') +
      (bestTip ? `\n\nTIP: ${bestTip}` : '');
  ...
  return (
    <div className="w-full space-y-4">
      {/* ========================================================
          1. ROUTE-OPTION SELECTOR (Pills & Segmented Cards)
          ======================================================== */}
                  ...
                  >
                    ...
                    <div className="mt-3 pt-2 border-t border-slate-100/30 text-xs font-body flex items-center justify-between gap-1">
                      <span className={isSelected ? 'text-slate-300 font-medium' : 'text-slate-500'}>
                        {opt.steps?.length} sakay · ~{optDuration}m
                      </span>
      ...
      {/* ========================================================
          2. RESPONSIVE DUAL-PANE GRID (Desktop / Laptop Widescreen)
          ======================================================== */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* ACTION BUTTONS (Copy, Save, New Search) */}
        ...
        {/* RIGHT COLUMN: DIRECTIONS TIMELINE (lg:col-span-5) */}
        <div className="lg:col-span-5 space-y-4">
          <div className="ios-card overflow-hidden">
            {/* Header: On Mobile it acts as a toggle, on Desktop it's always open and clearly visible */}
            <div
              onClick={() => setShowDirectionsMobile(!showDirectionsMobile)}
              className="w-full p-4 sm:p-5 flex items-center justify-between text-left lg:cursor-default cursor-pointer hover:bg-slate-50/80 lg:hover:bg-transparent transition-colors"
            >
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span className="text-xs font-body font-bold text-slate-500 bg-slate-100 px-2.5 py-0.5 rounded-full">
                  {activeSteps.length} sakay · ~{totalDuration}m
                </span>
              </div>
              <div className="lg:hidden inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-slate-100 text-slate-700 text-xs font-body font-semibold">
                <span>{showDirectionsMobile ? 'Itago' : 'Tingnan'}</span>
                {showDirectionsMobile ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
              </div>
            </div>

            {/* Directions Content (Always visible on Desktop lg:, collapsible on Mobile) */}
            <div className={`p-4 sm:p-5 pt-0 border-t border-slate-100 space-y-3 ${showDirectionsMobile ? 'block' : 'hidden lg:block'}`}>
              {/* Integrated Reminder Tip */}
              {bestTip && (
