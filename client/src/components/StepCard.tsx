  ...
  if (isWalking) {
    return (
      <div className={`relative flex items-start gap-3 ${!isLast ? 'pb-2.5' : ''}`}>
        ...
        {/* Compact Walking Description (No bulky box, no ₱0.00) */}
        <div className="flex-1 py-0.5 text-xs text-slate-500 font-body flex items-center gap-1.5">
  ...
  return (
    <div className={`relative flex items-start gap-3 ${!isLast ? 'pb-3.5' : ''}`}>
      ...
      <div className="flex flex-col items-center flex-shrink-0 w-3.5 pt-1.5">
        <div className="w-2.5 h-2.5 rounded-full bg-slate-800 shadow-xs" />
        {!isLast && (
      ...
      <div className="flex-1 bg-white border border-slate-200 rounded-lg p-3 shadow-xs">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap mb-1">
          <div className="flex items-center gap-1.5">
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-utility font-bold bg-slate-100 text-slate-800 border border-slate-200">
          ...
          {/* Fare: Only on paid legs, neutral utility font */}
          {hasFare && (
            <span className="font-utility text-xs font-bold text-slate-700">
